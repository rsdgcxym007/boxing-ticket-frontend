<template>
  <Teleport to="body">
    <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 overflow-auto">
      <div class="flex justify-center items-center min-h-screen p-6">
        <div
          class="w-full max-w-2xl bg-white dark:bg-zinc-800 backdrop-blur-md border border-white/10 rounded-3xl shadow-2xl p-6 space-y-8 relative"
        >
          <!-- ปุ่มปิด -->
          <button
            @click="$emit('update:showModal', false)"
            class="absolute top-4 right-4 text-white hover:text-red-400 text-xl"
          >
            ✕
          </button>
          ...
          <!-- หัวข้อ -->
          <h2
            class="text-2xl font-semibold text-center flex items-center justify-center gap-2 text-white"
          >
            <i class="mdi mdi-ticket-confirm-outline text-blue-400 text-3xl" />
            ออกตั๋วยืน
          </h2>
          ...
          <div class="flex gap-4">
            <div class="flex-1">
              <label
                class="text-sm mb-1 flex items-center gap-2 text-purple-300"
              >
...
watch(
  () => props.order,
  (newOrder) => {
    pageData.value = newOrder as any;
  },
  { immediate: true }
);
</script>
