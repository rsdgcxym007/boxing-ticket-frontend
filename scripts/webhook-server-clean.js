...
async function executeDeployment(webhookData) {
  try {
    ...
    await log(
      `Deployment initialization failed: ${error.message}`,
      "ERROR",
      true
    );
  }
...
const server = http.createServer(async (req, res) => {
  if (req.method !== "POST" || req.url !== "/hooks/deploy-frontend") {
    res.writeHead(200, { "Content-Type": "text/plain" });
    res.end(
      "Patong Boxing Webhook Server - POST /hooks/deploy-frontend to trigger deployment"
    );
    return;
  }
  ...
  req.on("end", async () => {
    try {
      ...
      if (event === "push" && payload.ref === "refs/heads/featues/v1") {
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ message: "Push ignored (not target branch)" })
        );
      } else {
...
server.listen(WEBHOOK_PORT, "0.0.0.0", async () => {
  await log(
    `GitHub Webhook Server started on port ${WEBHOOK_PORT}`,
    "INFO",
    true
  );
  await log(
    `GitHub webhook URL: http://43.229.133.51:${WEBHOOK_PORT}/hooks/deploy-frontend`,
    "INFO"
  );
  ...
  await sendDiscordNotification(
    `**Webhook Server Started**\n\n` +
      `**Port:** ${WEBHOOK_PORT}\n` +
      `**URL:** http://43.229.133.51:${WEBHOOK_PORT}/hooks/deploy-frontend\n` +
      `**Branch:** featues/v1\n` +
